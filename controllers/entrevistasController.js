  ...
  try {
    ...
    if (existingReservation && existingReservation.length > 0) {
      ...
      return res
        ...
        .json({
          success: false,
          message: `Ya tienes una entrevista agendada. Tu ficho de ingreso es: ${existingReservation[0].fich_entrevista}. Revisa tu correo.`,
        });
...
export const reserveInterviewSlot = async (req, res) => {
  try {
    ...
    const fich_entrevista = crypto.randomBytes(4).toString("hex").toUpperCase(); // 3. Registrar la reserva en la tabla `reservas_entrevista`
